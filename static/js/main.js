...
function createAnswerer(offer, peerUsername, receiver_channel_name){
	console.log('Answerer function activated.')
	var peer = new RTCPeerConnection(configuration); // the aurgumet configuration is defined above, its the stun server
	...
	peer.addEventListener('datachannel', e => {
		...
		// triggers when webRTC data channel receives a message
		peer.dc.addEventListener('messadge', dcOnMessage);
...
function dcOnMessage(event){
	...
	// checks when the peer connection status changes
	peer.addEventListener('iceconnectionstatechange', () => {
		// gets the peer connection status
		var iceConnectionState = peer.iceConnectionState;

		// checks for when the connection status of a peer changes to the below listed conditions.
		if(iceConnectionState === 'failed' || iceConnectionState === 'disconnected' || iceConnectionState === 'closed'){
			delete mapPeers[peerUsername];

			if(iceConnectionState != 'closed'){
				peer.close(); // closes / disconnects a peer connection when the iceConnection state is closed.
			}

			removeVideo() // removes video of a disconnected peer in the meeting.
		}
	})
}
...
btnJoin.addEventListener('click', () => {
	...
	// breaks the process if no username was submitted
	if(username==''){
		return;
	}
	...
	// triggers immidiately the websocket opens
	webSocket.addEventListener('open', (e) => {
		console.log('connection open')

		// signals all users that a new peer is connected
		sendSignal('new-peer', {})
	}); 
	...
	//when an error occure in the websocket
...
// gets the user local media stream
var localStream = new MediaStream();
// uuummm... I have to reassign the the source of this media to 'media from canvas' so I can manipulate it, be able to change to a custom background. make screen share a background.
